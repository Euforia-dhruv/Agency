...
class CarLights {
  ...
  init() {
    ...
    let colorObjs: THREE.Color | THREE.Color[];
    if (Array.isArray(this.colors)) {
      colorObjs = this.colors.map((c: number) => new THREE.Color(c));
    } else {
      colorObjs = new THREE.Color(this.colors);
    }
    ...
    for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
      ...
      const color = pickRandom(colorObjs) as THREE.Color;
...
class LightsSticks {
  ...
  init() {
    ...
    let stickColors: THREE.Color | THREE.Color[];
    if (Array.isArray(options.colors.sticks)) {
      stickColors = options.colors.sticks.map((c: number) => new THREE.Color(c));
    } else {
      stickColors = new THREE.Color(options.colors.sticks);
    }
    ...
    for (let i = 0; i < totalSticks; i++) {
      ...
      const color = pickRandom(stickColors) as THREE.Color;
      aColor.push(color.r, color.g, color.b);
...
class Road {
  ...
  createPlane(side: number, width: number, isRoad: boolean) {
    ...
    if (isRoad) {
      uniforms.uLanes = { value: options.lanesPerRoad };
      uniforms.uBrokenLinesColor = { value: new THREE.Color(options.colors.brokenLines) };
      uniforms.uShoulderLinesColor = { value: new THREE.Color(options.colors.shoulderLines) };
      uniforms.uShoulderLinesWidthPercentage = { value: options.shoulderLinesWidthPercentage };
      uniforms.uBrokenLinesLengthPercentage = { value: options.brokenLinesLengthPercentage };
      uniforms.uBrokenLinesWidthPercentage = { value: options.brokenLinesWidthPercentage };
    }
...
export default function Hyperspeed({ effectOptions = {} }: HyperspeedProps) {
  ...
  useEffect(() => {
    ...
    const mergedColors = { ...DEFAULT_EFFECT_OPTIONS.colors, ...(effectOptions.colors || {}) };
    const options: EffectOptions = {
      ...DEFAULT_EFFECT_OPTIONS,
      ...effectOptions,
      distortion:
        typeof effectOptions.distortion === "string" && distortions[effectOptions.distortion]
          ? distortions[effectOptions.distortion]
          : DEFAULT_EFFECT_OPTIONS.distortion,
      colors: mergedColors,
    };
  ...
  return <div ref={hyperspeed} id="hyperspeed" />;
}
